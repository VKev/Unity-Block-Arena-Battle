...
namespace PlayerStateMachine
{
    ...
    public class PlayerRootState : BaseState<PlayerStateMachine.EState>
    {
        ...
        [SerializeField] public float maxFloatingHeight = 0.4f;
        public float FloatingHeight { get; set; }

        [HideInInspector] public bool IsJumping;

        [HideInInspector] public bool IsTurnOffGrounded;
        ...
        {   
            Debug.Log("RootState jump" + IsJumping);
            Debug.Log("RootState gorund" + IsTurnOffGrounded);
            if(IsJumping == true){
                IsTurnOffGrounded = true;
            }else{
                IsTurnOffGrounded = false;
            }

            if (IsTurnOffGrounded == false){
                GroundSphereCast();
                ObstacleDetection();
            }
        }
        ...
        public override void OnDrawGizmos()
        {
            ...
            Gizmos.DrawWireSphere(ColliderBottomPoint, 0.01f);
            
            if (GroundRay.isHit)
            {
                Gizmos.color = Color.yellow;
                Gizmos.DrawLine(ColliderBottomPoint, GroundRay.hit.point);
                
                Gizmos.color = Color.red;
                Gizmos.DrawWireSphere(GroundRay.hit.point, 0.1f);
            }
            else
            {
                Gizmos.color = Color.yellow;
                Gizmos.DrawLine(ColliderBottomPoint, ColliderBottomPoint + Vector3.down * 10f);
            }
        }
    }
